...
import requests
from lxml import html  # pip install lxml, pip install cssselect


def main():
    parser = argparse.ArgumentParser(description='Find world-editable GitHub repository wikis.')
    parser.add_argument('--accounts_file', type=str, required=True,
                        help='Newline separated file of GitHub accounts to scan')
    parser.add_argument('--username', type=str,
                        help='GitHub username used for authentication. Required via argument or hardcoded.')
    parser.add_argument('--password', type=str,
                        help='GitHub password used for authentication. Required via argument or hardcoded.')
    ...
    if args.password:
        ...
        gitHubPassword = os.environ['GITHUB_ACCESS_TOKEN']
    ...
    with open(accountsFile) as accountsFileOpen:
        ...
        if accountTotal > rateLimit:
            print "\nWARNING - GitHub limits the number of requests you can make within a period of time. Your current limit is {0} requests but you are scanning {1} accounts.".format(
                rateLimit, accountTotal)
        else:
        ...
        for account in accountsList:
            ...
            try:
                repoRequest = gitHubSession.get(
                    "https://api.github.com/users/{0}/repos?per_page=100&page=1".format(account))
                ...
                if rateLimit < 2:
                    print "\nYour current GitHub request rate limit (number of accounts you can check): {0}".format(
                        rateLimit)
            ...
            for repoNum in range(repoTotal):
                try:
                    ...
                    if rateLimit == 0:
                        ...
                        if rateLimit429:
                            print "\nThe following {0} accounts and repos were blocked by a rate limit (429 response) and should be checked manually:".format(
                                len(rateLimit429))
                ...
                print "[*][Account {0}/{1}][Repo {2}/{3}] SCANNING wiki for {4}".format(accountCount, accountTotal,
                                                                                        repoNum + 1, repoTotal,
                                                                                        repoFullName)

                if outputFile:
                    outputFile.write(
                        "[*][Account {0}/{1}][Repo {2}/{3}] SCANNING wiki for {4}\n".format(accountCount, accountTotal,
                                                                                            repoNum + 1, repoTotal,
                                                                                            repoFullName))
    ...
    if editableWikiList:
        ...
        print "\nThe following {0} accounts and repos were found to have a world-editable wiki:".format(
            editableWikiTotal)
